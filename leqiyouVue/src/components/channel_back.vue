<template>
  <div class="channelBack">
    ...
    <div class="pt100">
      ...
      <ul class="righttab flex">
        <li class="ptr alic" v-for="v in activeChanList">
          <router-link :to="{path:'detail_zb',query:{channelID:v.channelID}}"  class="item">
            <img v-view="v.iconSmall" alt="">
            <p class="fs24 c33">{{v.channelName}}</p>
          </router-link>
        </li>
       </ul>
    </div>
...
<style scoped lang="scss">
 .channelBack {
    .righttab{
      padding: 0 .1rem;margin-left: 22%;width: 78%;flex-wrap:wrap;
      li{width:33.33%;padding-bottom: 33.33%;margin-bottom:.05rem;
        .item{padding: .2rem 0;position: absolute;top:.05rem;left:.05rem;right:.05rem;bottom: .05rem;border:1px solid #f2f2f2;
           img{width: .76rem;height: .76rem;}
        }
      }
